 ...
 * @param {Object} config.types     All registered content types.
 * @param {Object} config.schemas   All currently created schemas.
 * @return {Object} Express.Router() instance.
 */
function articlesRouter(config) {
  const router = express.Router();
  const { appConfig, types, schemas } = config;
  ...
  router.get('/', function(req, res) {
    ...
    if (req.query.slug) {
      ...
      // Only search for registered content types.
      query.content_type = { $in: types.map(type => type.id) };
  ...
  router.post('/', jwtValidate({ secret }), function(req, res) {
    ...
    if (!types.find(type => type.id === currentContentType)) {
      return res.status(500).send({
