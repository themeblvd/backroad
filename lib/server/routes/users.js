 ...
 */
function usersRouter(options, schemas) {
  ...
  router.put('/:username', (req, res) => {
    if (!req.user || (req.user.role !== 'admin' && req.user.username !== req.params.username)) {
      return res.status(500).send({ message: 'Not authorized.' });
    }
  ...
  router.delete('/:username', (req, res) => {
    ...
    User.findOneAndRemove({ username: req.params.username }, err => {
      if (err) return res.status(500).send(err);
      return res.status(201).send({ message: `User "${req.params.username}" was deleted.` });
    });
  });
