...
function backroad(config) {
  ...
  function start() {
    ...
    server.use('/api/v1/articles', articlesRouter(appConfig, contentTypes.get(), server.schemas));

    // Public article API. Mask content types within their own endpoints. Read-only.
    contentTypes.get().forEach(function(type) {
      server.use(`/api/v1/${type.endpoint}`, contentTypeRouter(type, server.schemas));
    });
