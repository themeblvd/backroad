...
const { toSlug } = require('./utils/formatting');
...
function backroad(config) {
  ...
  function addContentType(contentType) {
    ...
    if (!contentType.endpoint) {
      contentType.endpoint = toSlug(contentType.pluralName);
    }

    contentType.slug = toSlug(contentType.name);
    ...
    server.use(
      ...
      contentTypeRouter(
        { ...contentType, secret: backroadConfig.secret },
        createContentTypeSchema(contentType)
      )
    );
  }
   ...
   */
  function start() {
    const { secret, port } = backroadConfig;

    // Add secured configuration endpoints, for admin client.
    server.use('/api/v1/config', configRouter({ secret, contentTypes, optionGroups }));
