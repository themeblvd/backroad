...
function backroad(config) {
  ...
  function start() {
    ...
    server.use('/api/content-types', jwtValidate({ secret }));

    server.get('/api/content-types', function(req, res) {
      res.status(200).send(this.contentTypes);
    });

    contentTypes.forEach(function(type) {
      server.get('/api/content-types/:endpoint', function(req, res) {
