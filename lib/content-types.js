...
function createContentTypes(config) {
  ...
  function add(type) {
    ...
    const schema = createContentTypeSchema(type);

    server.schemas[type.id] = schema; // Store record for continuity with other schemas.

    server.use(`/api/v1/${type.endpoint}`, contentTypeRouter({ ...type, secret }, schema));
  }
